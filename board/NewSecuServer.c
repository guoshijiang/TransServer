...
void *NewSecuBoard(void *arg)
{
	...
	{	
		...
		{
			printf("新三方服务器端检测到客户端有一条连接已关闭 \n");
			break;
		}
		else if (ret == Sck_ErrTimeOut)
		{
			printf("新三方服务器端recv超时\n");
			continue;
		}
		else if (ret != 0)
		{
			printf("新三方服务器端 sckServer_rev() err\n");
			break;
		}
	
		printf("新三方收到的报文为out:%s \n", out);
		...
		{
			sck_FreeMem((void **)&out);
			printf("新三方服务器端send超时\n");
			continue;
		}
		else if (ret != 0)
		{
			sck_FreeMem((void **)&out);
			printf("新三方服务器端 sckServer_send() err\n");
			break;
		}
		sck_FreeMem((void **)&out);
	}
...
int main()
{
	...
	while (1)
	{
		...
		pthread_create(&pid, NULL,  NewSecuBoard, (void *)(connfd));

	}
	int sckServer_destroy();
	printf("NewSecuServer is End....\n");	
}