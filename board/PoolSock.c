...
typedef struct _SockePoolHandle
{
	...
	int				terminated;       //判断连接池是否已经终止: 1已经终止 0没有终止
}SockePoolHandle;
...
int sckCltPool_init(void **handle, SCKClitPoolParam *param)
{
	...
	if (hdl->valid < hdl->bounds ) //若有效连接数小于总数
	{
		ret = Sck_Err_Pool_CreateConn;
		Socket_Log(__FILE__, __LINE__,SocketLevel[4], ret,"有效连接数小于总数");
